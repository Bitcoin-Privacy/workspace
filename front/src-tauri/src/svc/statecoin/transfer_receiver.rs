...
use statechain_core::{
    transfer::receiver::{
        create_transfer_receiver_request_payload, get_new_key_info, validate_tx0_output_pubkey,
        verify_blinded_musig_scheme, verify_latest_backup_tx_pays_to_user_pubkey,
        verify_transaction_signature, verify_transfer_signature, GetMsgAddrResponsePayload,
        StatechainInfoResponsePayload, TransferReceiverRequestPayload, TxOutpoint,
    },
    utils::{get_blockheight, get_network, InfoConfig},
...
pub async fn execute(client_config: &ClientConfig, wallet_name: &str) -> Result<()> {
    ...
    for coin in wallet.coins.iter_mut() {
        ...
        println!(
            "statechain_id: {}",
            coin.statechain_id.as_ref().unwrap_or(&"".to_string())
        );
...
async fn process_encrypted_message(
    ...
    for enc_message in enc_messages {
        let transfer_msg = statechain_core::transfer::receiver::decrypt_transfer_msg(
            enc_message,
            &client_auth_key,
        )?;
        ...
        let tx0_outpoint = statechain_core::transfer::receiver::get_tx0_outpoint(
        ...
        let is_transfer_signature_valid =
            verify_transfer_signature(&new_user_pubkey, &tx0_outpoint, &transfer_msg)?;

        println!(
            "is_transfer_signature_valid: {}",
            is_transfer_signature_valid
        );
        ...
        let statechain_info =
            get_statechain_info(&transfer_msg.statechain_id, &client_config).await?;

        let is_tx0_output_pubkey_valid = validate_tx0_output_pubkey(
        ...
        let latest_backup_tx_pays_to_user_pubkey =
            verify_latest_backup_tx_pays_to_user_pubkey(&transfer_msg, &new_user_pubkey, network)?;

        println!(
            "latest_backup_tx_pays_to_user_pubkey: {}",
            latest_backup_tx_pays_to_user_pubkey
        );
        ...
        let is_tx0_output_unspent = verify_tx0_output_is_unspent_and_confirmed(
        ...
        for (index, backup_tx) in transfer_msg.backup_transactions.iter().enumerate() {
            ...
            if is_blinded_musig_scheme_valid.is_err() {
                println!(
                    "{}",
                    is_blinded_musig_scheme_valid.err().unwrap().to_string()
                );
                sig_scheme_validation = false;
                break;
            }
        ...
        let transfer_receiver_request_payload =
            create_transfer_receiver_request_payload(&statechain_info, &transfer_msg, &coin)?;

        let server_public_key_hex = send_transfer_receiver_request_payload(
            &client_config,
...
async fn verify_tx0_output_is_unspent_and_confirmed(
    ...
    let output_address = statechain_core::transfer::receiver::get_output_address_from_tx0(
        &tx0_outpoint,
        &tx0_hex,
        &network,
    )?;
...
async fn send_transfer_receiver_request_payload(
    client_config: &ClientConfig,
    transfer_receiver_request_payload: &TransferReceiverRequestPayload,
) -> Result<String> {
