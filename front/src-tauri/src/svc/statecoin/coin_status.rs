...
use statechain_core::wallet::{Activity, BackupTx, Coin, CoinStatus};
...
pub async fn check_deposit_cp(
    ...
    if coin.status == CoinStatus::INITIALISED {
        ...
        // NOTE: Create backup txn
        let backup_tx =
            create_tx1(client_config, coin, wallet_netwotk, &utxo_txid, utxo_vout).await?;
...
async fn check_transfer(client_config: &ClientConfig, coin: &Coin) -> Result<bool> {
    ...
    let is_transfer_complete = response
