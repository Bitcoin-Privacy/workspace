...
impl BlindSession {
    pub fn from_k(k: &str) -> Result<Self> {
        let k_bytes: [u8; 32] = hex::decode(k)
            .expect("Cannot parse k from hex")
            .try_into()
            .expect("K got invalid format");
    ...
    pub fn sign_ep(self, ep: &[u8; 32], xs: Scalar) -> Result<[u8; 32]> {
        let scalar: Option<Scalar> = Scalar::from_canonical_bytes(*ep).into();
        Ok((xs * scalar.ok_or(WiredScalarMalformed)? + self.k).to_bytes())
    }
}
