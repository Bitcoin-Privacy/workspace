...
impl BlindRequest {
    ...
    {
        let mut rng = OsRng;
        initiate::<H, &[u8; 32]>(rp, Scalar::random(&mut rng).as_bytes())
    }
    ...
    pub fn gen_signed_msg(self, sp: &[u8; 32]) -> Result<UnblindedSigData> {
        let scalar: Option<Scalar> = Scalar::from_canonical_bytes(*sp).into();
        let sp = scalar.ok_or(WiredScalarMalformed)?;
