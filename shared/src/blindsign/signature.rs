...
impl TryFrom<&str> for WiredUnblindedSigData {
    type Error = String;

    fn try_from(value: &str) -> std::result::Result<Self, Self::Error> {
        let sig_data: [u8; 96] = hex::decode(value)
            .map_err(|_| "Invalid signature format".to_string())?
            .try_into()
            .map_err(|_| "Invalid signature size".to_string())?;
        Ok(WiredUnblindedSigData(sig_data))
    }
}
...
impl WiredUnblindedSigData {
    ...
    pub fn to_internal_format(&self) -> Result<UnblindedSigData> {
        ...
        let option_e: Option<Scalar> = Scalar::from_canonical_bytes(e_arr).into();
        let option_s: Option<Scalar> = Scalar::from_canonical_bytes(s_arr).into();
        Ok(UnblindedSigData {
            e: option_e.ok_or(WiredScalarMalformed)?,
            s: option_s.ok_or(WiredScalarMalformed)?,
